...
export default function Contact() {
  ...
  const [sendRipples, setSendRipples] = useState<{ id: number; x: number; y: number; size: number }[]>([]);

  const addSendRipple = useCallback((e: React.MouseEvent) => {
    if (!sendBtnRef.current) return;
    const rect = sendBtnRef.current.getBoundingClientRect();
    const size = Math.max(rect.width, rect.height);
    const id = Date.now();
    setSendRipples((prev) => [...prev, { id, x: e.clientX - rect.left - size / 2, y: e.clientY - rect.top - size / 2, size }]);
    setTimeout(() => setSendRipples((prev) => prev.filter((r) => r.id !== id)), 650);
  }, []);
    ...
    <section id="contact" className="relative py-10 sm:py-14 xl:py-16 overflow-hidden">
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none">
        <img
          ...
          style={{ transform: "translateX(-109%) translateY(-90px)" }}
        />
      </div>
        ...
        <FadeIn delay={0.1}>
          <form onSubmit={handleSubmit} className="text-left space-y-4 mb-8" noValidate>
            ...
            <button
              ...
              type="submit"
              disabled={sending}
              onClick={addSendRipple}
              className="kinetics-ripple btn-hover inline-flex items-center justify-center px-8 py-3 bg-accent text-background text-sm font-medium rounded-xl hover:opacity-90 transition-opacity min-h-[44px] min-w-[44px] disabled:opacity-50"
        ...
        <FadeIn delay={0.15}>
          <div className="flex items-center justify-center gap-6 mb-8">
            <a
              ...
              className="kinetics-social social-email flex items-center gap-2 text-text-2 min-h-[44px] min-w-[44px] justify-center"
            >
              <Mail size={20} />
            ...
            <a
              ...
              className="kinetics-social social-github flex items-center gap-2 text-text-2 min-h-[44px] min-w-[44px] justify-center"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="kinetics-social social-linkedin flex items-center gap-2 text-text-2 min-h-[44px] min-w-[44px] justify-center"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="kinetics-social social-facebook flex items-center gap-2 text-text-2 min-h-[44px] min-w-[44px] justify-center"
            >
